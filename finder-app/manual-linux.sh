...
export PATH=$PATH:/home/ubuntu/install/aarch/arm-gnu-toolchain-13.3.rel1-x86_64-aarch64-none-linux-gnu/bin:
...
${CROSS_COMPILE}readelf -a ${OUTDIR}/rootfs/bin/busybox | grep "Shared library" | awk '{print $NF}' | tr -d '[]' | while read library; do
    find ${SYSROOT} -name "${library}" | xargs -I {} cp -L {} ${OUTDIR}/rootfs/lib64/
done
...
| gzip -9 > "${OUTDIR}/initramfs.cpio.gz"
